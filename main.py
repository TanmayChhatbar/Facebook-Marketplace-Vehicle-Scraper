...
def main():
    # open page
    url = "https://www.facebook.com/login/device-based/regular/login/?login_attempt=1&next=https%3A%2F%2Fwww.facebook.com%2Fmarketplace%2F"
    ...
    with open(f"sc_{fname}.csv", 'w', newline='') as f,\
        ...
        for year, name, price, mileage, loc, link in zip(
            years, names, prices, mileages, locs, links
        ):
            csw.writerow([year, name, price, mileage, loc, link])
            if (
                setup["facebook"]["carBrand"].lower() in name.lower()
            ):  # if car name is in name
                ...
                if str(mileage) in {"Dealership", "N/A"}:
                    ...
                    setup["facebook"]["desired_maximum_mileage"]
                ) and int(year) > (setup["facebook"]["desired_minimum_year"]):
